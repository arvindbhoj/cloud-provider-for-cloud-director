...
import (
	...
	"github.com/vmware/cloud-provider-for-cloud-director/pkg/go-vcloud-director/v2/types/v56"
	"github.com/vmware/cloud-provider-for-cloud-director/pkg/go-vcloud-director/v2/util"
	"k8s.io/klog"
)
...
//Arvind Bhoj - The session time seems to be many hours. To keep things simple checking when the session was created
// and recreating if it was more than x hours back
func isLessThanThreeHoursOld(t time.Time) bool {
	return time.Now().Sub(t) < 3*time.Hour
}

//Arvind Bhoj - Customized this to only authorize if session not already cached
...
func (vcdClient *VCDClient) vcdCloudApiAuthorize(user, pass, org string) (*http.Response, error) {
	...
	//Arvind Bhoj - Moved these to the top as we want to set them if cached credentials are still valid
	...
	//Arvind Bhoj - Will add this if we want to do a request and check if token is good
	// For now just setting vcdClient.Client.VCDToken and returning blank resp
	if file, err := os.Stat(fmt.Sprint("/tmp/token-", org, "-", user)); err == nil {
		if isLessThanThreeHoursOld(file.ModTime()) {
			token, err := os.ReadFile(fmt.Sprint("/tmp/token-", org, "-", user))
			if err != nil {
				return nil, fmt.Errorf("Something went wrong. Error reading token data")
			}
			vcdClient.Client.VCDToken = string(token)
			klog.Infof("Token is cached for user [%s] and org [%s] and still valid Not attempting to login again", user, org)
			rawUrl := vcdClient.sessionHREF.Scheme + "://" + vcdClient.sessionHREF.Host + "/cloudapi/1.0.0/orgs"
			getOrgsUrl, err := url.Parse(rawUrl)
			req := vcdClient.Client.NewRequest(map[string]string{}, http.MethodGet, *getOrgsUrl, nil)
			req.Header.Add("Accept", "application/*;version="+vcdClient.Client.APIVersion)
			req.Header.Add("Content-Type", "application/json")
			req.Header.Add("Authorization", "Bearer "+vcdClient.Client.VCDToken)
	...
	//Arvind Bhoj - Moving these to the top to perform a query to check validity of cached token
	//vcdClient.Client.VCDAuthHeader = BearerTokenHeader
	//vcdClient.Client.IsSysAdmin = strings.EqualFold(org, "system")
	// Get query href
	//vcdClient.QueryHREF = vcdClient.Client.VCDHREF
...
func (vcdClient *VCDClient) GetAuthResponse(username, password, org string) (*http.Response, error) {
	...
	f, err := os.OpenFile(fmt.Sprint("/tmp/token-", org, "-", username), os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		util.Logger.Printf("[ERROR]: %s", err)
	}
	defer f.Close()
	klog.Infof("Generating token for [%s]-[%s]", org, username)
